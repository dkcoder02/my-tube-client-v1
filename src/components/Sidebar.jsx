...
export default function Sidebar({
  defaultStyle = "lg:sticky lg:max-w-[250px]",
  defaultSpanStyle = "lg:inline",
}) {
  ...
  const handleChannelData = async () => {
    try {
      ...
      if (username && userId) {
        ...
        if (ownChannelVideos)
          ownChannelVideos.data.data.videos.length !== 0
            ? dispatch(isEmptyVideos(false))
            : dispatch(isEmptyVideos(true));
      }
    } catch (error) {
  ...
  return (
    <aside
      className={`group fixed inset-x-0 bottom-0 z-40 w-full shrink-0 border-t border-white bg-[#121212] px-2 py-2 sm:absolute sm:inset-y-0 sm:max-w-[70px] sm:border-r sm:border-t-0 sm:py-6 sm:hover:max-w-[250px] ${defaultStyle}`}
    >
        ...
        <li className="">
          <Link to="/">
            <button className="flex flex-col items-center justify-center border-white py-1  focus:text-[#e9bb65] sm:w-full sm:flex-row sm:border sm:p-1.5 sm:hover:bg-[#e9bb65] sm:hover:text-black sm:focus:border-[#e9bb65] sm:focus:bg-[#e9bb65] sm:focus:text-black sm:group-hover:justify-start sm:group-hover:px-4 lg:justify-start lg:px-4">
              ...
              <span
                className={`block sm:hidden sm:group-hover:inline ${defaultSpanStyle}`}
              >
                Home
              </span>
            </button>
