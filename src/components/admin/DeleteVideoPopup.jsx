...
function DeleteVideoPopup({
  ...
  const handleDeleteVideo = async () => {
    try {
      setLoading(true);
      const deleteVideo = await videoService.deleteVideo({ videoId });
      if (deleteVideo && deleteVideo.data?.success === true) {
        setDeleteVideoModelActive(false);
        setReload(!reload);
      }
    } catch (error) {
        ...
        <div className="mx-auto w-full max-w-lg overflow-auto rounded-lg border border-gray-700 bg-[#121212] p-4">
          <div className="mb-6 flex items-start gap-4">
            ...
            <button
              ...
              onClick={() => setDeleteVideoModelActive(false)}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
